...
import dr.evomodel.arg.operators.ARGExchangeOperator;
import dr.evomodel.arg.operators.ARGPartitioningOperator;
import dr.evomodel.arg.operators.ARGSubtreeSlideOperator;
...
//            The actual mutation rate is now in the BranchRateModel so relativeRate can be missing
	...
	public void writeBranchRatesModel(XMLWriter writer) {
		if (clockModel == STRICT_CLOCK) {
			...
			writer.writeOpenTag(
					ARGDiscretizedBranchRates.DISCRETIZED_BRANCH_RATES,
					new Attribute[]{new Attribute.Default("id", "branchRates")}
			);
			...
			writer.writeCloseTag(ARGDiscretizedBranchRates.DISCRETIZED_BRANCH_RATES);
			...
			writer.writeTag(TreeModel.TREE_MODEL, new Attribute.Default("idref", "treeModel"), true);
			writer.writeTag(ARGDiscretizedBranchRates.DISCRETIZED_BRANCH_RATES, new Attribute.Default("idref", "branchRates"), true);
	...
	private void writeTossPartitioningOperation(Operator operator, XMLWriter writer) {
		...
		writer.writeOpenTag(ARGPartitioningOperator.OPERATOR_NAME,
				new Attribute[]{
						new Attribute.Default("weight", Integer.toString((int) operator.weight)),
						//new Attribute.Default(ARGPartitioningOperator.IS_RECOMBINATION, "false")
				});
		writer.writeTag(ARGModel.TREE_MODEL, new Attribute[]{new Attribute.Default("idref", "treeModel")}, true);
		writer.writeCloseTag(ARGPartitioningOperator.OPERATOR_NAME);
	}

	private void writeAddRemoveARGEventOperation(Operator operator, XMLWriter writer) {
		writer.writeOpenTag(ObsoleteARGAddRemoveEventOperator.SUBTREE_SLIDE,
				new Attribute[]{
						new Attribute.Default("weight", Integer.toString((int) operator.weight)),
						new Attribute.Default("gaussian", "true"),
						new Attribute.Default("size", Double.toString(operator.tuning)),
		...
		writer.writeTag(ARGModel.TREE_MODEL, new Attribute[]{new Attribute.Default("idref", "treeModel")}, true);
		writer.writeOpenTag(ObsoleteARGAddRemoveEventOperator.JUST_INTERNAL);
		writer.writeTag(ParameterParser.PARAMETER, new Attribute.Default("idref", "treeModel.internalNodeHeights"), true);
		writer.writeCloseTag(ObsoleteARGAddRemoveEventOperator.JUST_INTERNAL);
		writer.writeOpenTag(ObsoleteARGAddRemoveEventOperator.INTERNAL_AND_ROOT);
		...
		writer.writeCloseTag(ObsoleteARGAddRemoveEventOperator.INTERNAL_AND_ROOT);
		writer.writeCloseTag(ObsoleteARGAddRemoveEventOperator.SUBTREE_SLIDE);
	}
	...
	public void writeMCMC(XMLWriter writer) {
		...
		if (nodeHeightPrior == YULE || nodeHeightPrior == BIRTH_DEATH) {
			...
			writer.writeTag(VeryOldCoalescentLikelihood.COALESCENT_LIKELIHOOD, new Attribute.Default("idref", "coalescent"), true);
		}
