...
//import dr.evomodel.tree.ARGModel.NodeHeightBounds;
        ...
        implements MutableTree
{
   ...
   //     initialize(tree);
     ...
     * tree.
     */
    public final int getNodeCount() { return nodes.size(); }
    ...
    public final boolean isBifurcation(NodeRef node) { return ((Node)node).isBifurcation(); }
    public final boolean isBifurcationDoublyLinked(NodeRef node) { return ((Node)node).isBifurcationDoublyLinked();}
    public final boolean isReassortment(NodeRef node) { return ((Node)node).isReassortment(); }
    ...
    public final NodeRef getChild(NodeRef node, int i, int partition) { return ((Node)node).getChild(i,partition); }
...
//    
    ...
    public final NodeRef getExternalNode(int i) { return nodes.get(i); }
    public final NodeRef getInternalNode(int i) { return nodes.get(i+externalNodeCount); }
    ...
    public void addChildAsRecombinant(NodeRef p1, NodeRef p2, NodeRef c, BitSet bs1, BitSet bs2) {
    	...
    	parent1.addChildRecombinant(child, bs1);
    	parent2.addChildRecombinant(child, bs2);
...
//System.exit(-1);
    ...
    protected void restoreState() {
        ...
        if( addedParameters != null ) {
        	storedNodeHeights.removeParameter(addedParameters[0]);
        	storedNodeHeights.removeParameter(addedParameters[1]);
        	removeParameter(addedParameters[0]);
        	removeParameter(addedParameters[1]);
        	//reinitializeLikelihood();
        }
        if( removedParameters != null) {
        	storedNodeHeights.addParameter(removedParameters[0]);
        	storedNodeHeights.addParameter(removedParameters[1]);
    ...
    public void addNewHeightParameters(Parameter newbie1, Parameter newbie2, 
    	...
    	storedNodeHeights = internalNodeParameters;
    	storedNodeHeights.addParameter(newbie1);
    	storedNodeHeights.addParameter(newbie2);
    }
    ...
    public void expandARGWithRecombinant(Node newbie1, Node newbie2, 
    	...
    	storedNodeHeights = internalNodeParameters;
    	storedNodeHeights.addParameter(newbie1.heightParameter);
    	storedNodeHeights.addParameter(newbie2.heightParameter);
    	nodes.add(newbie1);
    	nodes.add(newbie2);
     	internalNodeCount += 2;
    ...
    public void contractARGWithRecombinant(Node oldie1, Node oldie2,
    	...
    	storedNodeHeights = internalNodeParameters;
    	storedNodeHeights.removeParameter(oldie1.heightParameter);
    	storedNodeHeights.removeParameter(oldie2.heightParameter);
    	nodes.remove(oldie1);
    	nodes.remove(oldie2);
    	internalNodeCount -= 2;   	
    }
    ...
    public int addTaxon(Taxon taxon) { throw new IllegalArgumentException("Cannot add taxon to a ARGModel"); }
    public boolean removeTaxon(Taxon taxon) { throw new IllegalArgumentException("Cannot add taxon to a ARGModel"); }
    public void setTaxonId(int taxonIndex, String id) { throw new IllegalArgumentException("Cannot set taxon id in a ARGModel"); }
    public void setTaxonAttribute(int taxonIndex, String name, Object value) { throw new IllegalArgumentException("Cannot set taxon attribute in a ARGModel"); }
    ...
    public static XMLObjectParser PARSER = new AbstractXMLObjectParser() {
        ...
        public Class getReturnType() { return ARGModel.class; }
...
//        	node.parent = null;
        ...
        public boolean isRoot() { return (leftParent == null && rightParent == null); }
        public boolean hasEqualChildren() { return (leftChild == rightChild); }

        public boolean isBifurcation() { return bifurcation; }
        //public boolean isReassortment() { return hasChildren() && (leftChild == rightChild); }
        public boolean isReassortment() { return !bifurcation; }
    ...
    protected VariableSizeCompoundParameter storedNodeHeights;
