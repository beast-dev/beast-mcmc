...
\citep{HR2001}. This enthusiasm can be attributed to a number of factors.
...
of demographic parameters from genetic data \citep{Beaumont1999,DNRS2002,WWB2003,RY2003,PDNRR2003,Kuhner2006}.
...
of phylogeny and alignment \citep{LMDJH2005,RS2005}. Taken together
...
similar goals, such as \textbf{\textsc{MrBayes}} \citep{HR2001}, which
...
in BEAST is Metropolis-Hastings MCMC \citep{MRRTT1953,Hastings1970}.
...
clock model \citep{ZP1965}) with this rate known in advance or estimated
...
lineages\citep{TKP1998,YY2000,KTB2001,Sanderson2002,TK2002,AY2003}.
BEAST was the first software package that allows phylogenetic inference
under such models \citep{DHPR2006}. 
...
as \textbf{\textsc{MrBayes}} \citep{HR2001} and \textbf{\textsc{BAli-Phy}}
\citep{SR2006}.
\end{itemize}
...
to 1994 (see \citep{Rambaut2000} for details). Like many RNA viruses, Dengue virus evolves at a rapid rate and as a result the sampling times of the 17 isolates can be
...
known HKY85 model \citep{HKY1985} - can be specified. For the analysis
...
protein-coding sequence with introns removed) the Goldman and Yang model \citep{GY1994} can be used to model
codon evolution. 

In addition, both $\Gamma$-distributed rates among sites \citep{Y1994}
...
strict molecular clock model \citep{ZP1965}, calibrated by specifying
...
about the overall rate of evolutionary change \citep{DNRS2002,DPRFR2003}.
...
such as exponential or lognormal \citep{DHPR2006}.
...
In addition, the highly parametric Bayesian skyline plot \citep{DRSP2005}
...
contains more information on calibration priors \citep{DHPR2006}. 
...
\citep{TKF1991} is available. Interested readers should refer to the
paper on this subject for more details \citep{LMDJH2005}.
...
HKY85 model of nucleotide substitution \citep{HKY1985}. This parameter only appears if you have selected the HKY
substitution model.

\item [\texttt{\small{siteModel.alpha}}]	 This parameter is the shape ($\alpha$) parameter of the $\Gamma$
distribution of rate heterogeneity among sites \citep{Y1994}. This parameter only appears when you have selected Gamma
...
the breadth of application the software enjoys \citep{PDNRR2003,Lemeyetal2004,Shapiroetal2004,DRSP2005,LMDJH2005}. 
...
we would like to thank Roald Forsberg, Joseph Heled, Oliver Pybus, Beth Shapiro, 
Korbinian Strimmer and Marc Suchard for invaluable contributions. Thanks to Sidney
...
\bibitem{Rambaut2000}Rambaut A: Estimating the rate of molecular evolution: incorporating non-contemporaneous sequences into maximum likelihood phylogenies. \emph{Bioinformatics} 2000, \textbf{16}:395-399.
...
\bibitem{Shapiroetal2006}Shapiro B, Rambaut A, Drummond AJ: 
Choosing Appropriate Substitution Models for the Phylogenetic Analysis of 
Protein-Coding Sequences \emph{Mol Biol Evol}
2006, \textbf{23}(1):7-9.
