...
non dated datasets whatever time units the rate/date priors are specified in).  These taxon subsets can represent
...
to other taxa in the resulting MCMC analysis. Therefore the corresponding MRCA may in fact contain other descendents
...
from the dataset to be extracted.  In BEAUti, you can only partition into 1st,. 2nd and 3rd codon positions.  All other
...
zero then there is among branch rate heterogeneity within you data, and we recommend the use of a relaxed molecular
clock.
...
example of such a parameter is \xml{hky.kappa} which though efficiently estimated, requires a complete recalculation of the likelihood of the data whenever it is changed. Giving these parameters lower weights can improve the computational
...
\xml{treeModel}.  For example, if you are analyzing $x$ sequences, a very rough rule of thumb is that you should set the
...
The "auto optimize" option will automatically adjust the tuning parameters of operators as the MCMC algorithm runs, to try to achieve maximum efficiency.  We recommend that you choose this option. The criterion that our auto-optimization
...
\item [{\it Covariance}] If this value is strongly positive, then it means that within your phylogeny, branches with fast rates are followed by branches with fast rates.  If this value spans zero, then bransches with fast rates and slow rates are next to each other.  It also means that there is no strong evidence of autocorrelation of rates in the phylogeny.  Fits where???
...
This option allows you to resample your posterior distribution at a lower frequency than in previous BEAST runs.  Why important?
\subsection{Select input files}
Here you can select using the "+" button the input files that you wish to combine.  These will appear in the sub-window with the file name and the BurnIn peroid (by default 10%).
\subsection{Output file}
This options allows you to select a log file or create a new log file that the combined log data will be saved to.

When you click "Run" the log files you have selected will be combined in the JAVA LogCombiner window.  The files you have selected must be from independent runs of BEAST from the same XML file otherwise an error will occur stating that the number of columns in the first file does not match that of the second file.  

Once LogCombiner has finished you can analyse the combined log file in Tracer.

\section{TreeAnnotator}
This program is for summarizing the information in a sample of tree produced by BEAST e.g. annotating the probability of nodes having a rate change.  Once again, when you open this program the TreeAnnotator user interface window and the JAVA TreeAnnotator window will appear.
\subsection{BurnIn}
This option allows you to select the amount of BurnIn i.e. the number of samples that will be discarded at the start of the run, so that you are only analysing data that has reached equilibrium.
\subsection{Posterior probability limit}
This is the same as specifying a a limit for bootstraping in PAUP.  It will only show branches within the tree that have a posterior probability of greater than 0.5, and collaspe any branches into polytonies that have a posterior probability limit of less than 0.5.
\subsection{Target tree type}
Maximum clade credibility and User target tree.  Explain?
\subsection{Node heights}
This option allows you select how the node heights are summarised on the the tree, whether you keep the target heights, or display mean or median heights.
...
This option allows you to select the input tree file that you specified tree data to be saved to in BEAUti.
\subsection{Output file}
