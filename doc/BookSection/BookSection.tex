...
to each other. Some parameters tend to be estimated very efficiently - an example is the kappa 
...
Tracer. The aim is to set the chain length so as to achieve a reasonable effective sample 
size (ESS).
...
At this point we are ready to generate a BEAST XML file and to use this to run BEAST and 
perform the analysis. To do this, either select the {\it Generate BEAST File...} option 
...
convienience, leave teh BEAUti window open so that you can change the values and re-generate
the BEAST file as required later in this tutorial.

\section{Analysing the BEAST output}

To analyse the results of running BEAST we are going to use the program Tracer. This
is available from the BEAST website. The exact instructions for running Tracer differs depending 
...
ESS (for the 'clock.rate') is 26, it would suggest that we have to run it for 4 
...
parameters of interest: mutation rate. Select siteModel1.mu in the left-hand table. This 
...
We seen how we can diagnose our MCMC run using Tracer and produce estimates of the
...
trees. The tool is called 'TreeAnnotator' and once again, the details for running 
...
\item Posterior probability limit
...
{\it Branch Labels} title. You can then increase the font size here and possibly reduce
