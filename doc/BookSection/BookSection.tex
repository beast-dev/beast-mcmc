...
\textbf{\textsc{BEAST}} can be compared to a number of other software packages with
...
for MCMC analysis. By separating the graphical user interface (\textbf{\textsc{BEAUti}})
from the analysis (\textbf{\textsc{BEAST}}) we accommodate an XML layer that captures
...
\begin{itemize}
\item \textbf{\textsc{BEAST}} - this package contains the BEAST program, BEAUti and a couple of utility programs. At the time of writing,
the current version is v1.4.4. It is available for download from http://beast.bio.ed.ac.uk/.
\item \textbf{\textsc{Tracer}} - this program is used to explore the output of BEAST (and other Bayesian MCMC programs). It
...
\item \textbf{\textsc{FigTree}} - this is an application for displaying and printing molecular phylogenies, in particular
...
To load a NEXUS format alignment, simply select the \texttt{Import NEXUS...} option from the \texttt{File} menu. The examples
folder contains a file called \texttt{Dengue4.env.nex}. This file contains an alignment of 17 sequences from the
{\it E} gene of dengue virus, 1485 nucleotides in length.
...
\begin{center}
\leavevmode
\scalebox{0.4}{\includegraphics{Figure1_Data.png}}
\end{center}
...
In this case, the dengue virus have been sampled at various dates going back to the 1950s. The actual year of 
...
sequences in BEAUTi is to use the \texttt{Guess Dates} button at the top of the \texttt{Data} tab. Clicking 
this will make a  dialog box appear (Figure~\ref{fig:figure2}).

\begin{figure}[htbp]
\begin{center}
\leavevmode
\scalebox{0.4}{\includegraphics{Figure2_GuessDates.png}}
\end{center}
\caption{The Guess Dates dialog}
\label{fig:figure2}
\end{figure}

This operation attempts to guess what the dates are from information contained within the taxon names. Generally 
this works by trying to find a numerical field within each name. If the taxon names contain more than one 
numerical field (such as the dengue sequences, above) then you can specify how to find the one that 
...
before the date field in each name). For the dengue virus sequences you select last from the drop-down menu 
...
In this dialog box, you can also get \textbf{\textsc{BEAUti}} to add a fixed value to each guessed date. In this case the 
...
The next thing to do is to click on the \texttt{Model} tab at the top of the main window. This will reveal the 
...
shows the settings that will appear after loading the dengue virus data.

\begin{figure}[htbp]
\begin{center}
\leavevmode
\scalebox{0.4}{\includegraphics{Figure3_Model.png}}
\end{center}
...
a couple of points to note about selecting a model in \textbf{\textsc{BEAUti}}:

\begin{itemize}
\item Selecting the \texttt{Partition into codon positions} option assumes that the data are aligned 
...
\item Selecting the \texttt{Unlink substitution model across codon positions} will specify that BEAST 
should estimate a separate transition-transversion ratio or general time reversible rate matrix for 
each codon position.
\item Selecting the \texttt{Unlink rate heterogeneity model across codon positions} will specify that 
...
fixed \texttt{mean substitution rate} obtained from another source. Setting this to 1.0 will result 
in the ages of the nodes of the tree being estimated in units of substitutions per site (i.e. the 
normal units of branch lengths in popular packages such as \textbf{\textsc{MRBAYES}}).
\end{itemize}
...
proposals by other MCMC software packages such as \textbf{\textsc{MRBAYES}} and \textbf{\textsc{LAMARC}}). 
The operators specify how 
the parameters change as the MCMC runs. This table lists the parameters, their operators and the 
tuning settings for these operators. In the first column are the parameter names. These will be 
called things like \texttt{hky.kappa} which means the HKY model's kappa parameter (the 
...
The next column, labelled \texttt{Tuning}, gives a tuning setting to the operator. Some operators 
don't have any tuning settings so have \texttt{n/a} under this column. The tuning parameter will 
...
the window is an option called \texttt{Auto Optimize} which, when selected, will automatically adjust 
...
The next column, labelled \texttt{Weight}, specifies how often each operator is applied relative 
to each other. Some parameters tend to be estimated very efficiently - an example is the \texttt{kappa} 
...
The \texttt{MCMC} tab in \textbf{\textsc{BEAUti}} provides settings to control the MCMC chain 
(Figure~\ref{fig:figure4}). Firstly we have the 
...
\textbf{\textsc{Tracer}}. The aim is to set the chain length so as to achieve a reasonable effective sample 
size (ESS).
...
\begin{center}
\leavevmode
\scalebox{0.4}{\includegraphics{Figure4_MCMC.png}}
\end{center}
...
At this point we are ready to generate a BEAST XML file and to use this to run \textbf{\textsc{BEAST}} and 
perform the analysis. To do this, either select the \texttt{Generate BEAST File...} option 
...
convienience, leave the \textbf{\textsc{BEAUti}} window open so that you can change the values and re-generate
the BEAST file as required later in this tutorial.

\section{Analysing the BEAST output}

To analyse the results of running BEAST we are going to use the program \textbf{\textsc{Tracer}}. This
is available from the BEAST website. The exact instructions for running \textbf{\textsc{Tracer}} differs depending 
on which computer you are using. Please see the README text file that was distributed with the 
version you downloaded. Once running, \textbf{\textsc{Tracer}} will look similar irrespective of which computer system 
it is running on. 
...
\item \texttt{Stdev}
The standard deviation of the mean. This takes into account the effective sample size so a 
small ESS will give a large Stdev.
\item \texttt{Median}
The median value of the sampled trace across the chain (excluding the burn-in).
\item \texttt{95\% HPD Lower}
The lower bound of the highest posterior density (HPD) interval. The HPD is a credible set 
that contains 95\% of the sampled values.
\item \texttt{95\% HPD Upper}
The upper bound of the highest posterior density (HPD) interval. The HPD is a credible set 
that contains 95\% of the sampled values.
\item \texttt{Auto-Correlation Time (ACT)}
The number of states in the MCMC chain that two samples have to be from each other for them 
to be uncorrelated. The ACT is estimated from the samples in the trace (excluding the burn-in).
\item \texttt{Effective Sample Size (ESS)}
...
If we select the tab on the right-hand-side labelled \texttt{Trace} we can view the raw trace, 
...
ESS (for the \texttt{clock.rate}) is 26, it would suggest that we have to run it for 4 
...
and create a new BEAST XML file with a longer chain length. Now run \textbf{\textsc{BEAST}} and load the new 
log file into \textbf{\textsc{Tracer}} (you can leave the old one loaded for comparison). Click on the \texttt{Trace} 
...
We seen how we can diagnose our MCMC run using \textbf{\textsc{Tracer}} and produce estimates of the
posterior distributions of parameters of our model. However, BEAST also samples
...
trees into a program such as \textbf{\textsc{PAUP}} and construct a consensus tree in a similar manner
...
trees. The tool is called \textbf{\textsc{TreeAnnotator}} and once again, the details for running 
the program will differ depending on which operating system you are using. Once
running, you will be presented with a window like the one in Figure~\ref{fig:figure9}.

\begin{figure}[htbp]
\begin{center}
\leavevmode
\scalebox{0.4}{\includegraphics{Figure9_TreeAnnotator.png}}
\end{center}
\caption{The user-interface for the TreeAnnotator tool.}
\label{fig:figure9}
\end{figure}

\textbf{\textsc{TreeAnnotator}} takes a single 'target' tree and annotates it with the summarized
...
\item \texttt{Posterior probability limit}
...
consensus tree). Set this value to 0.0 to summarize all nodes.
\item \texttt{Target tree type}
...
the highest sum of the posterior probability of all its nodes. 
\item \texttt{Node heights}
...
\item \texttt{Target Tree File}
If the "User target tree" option is selected then you can use "Choose File..." to select a
NEXUS file containing the target tree.
\item \texttt{Input Tree File}
Use the "Choose File..." button to select an input trees file. This will be the trees file
produced by BEAST.
\item \texttt{Output File}
Select a name for the output tree file.
\end{itemize}

Once you have selected all the options, above, press the \texttt{Run} button. \textbf{\textsc{TreeAnnotator}} will analyse
the input tree file and write the summarized tree to the file you specified. This tree is in
...
Run \textbf{\textsc{FigTree}} now and select the \texttt{Open...} command from the \texttt{File} menu. Select the tree file
...
First open the \texttt{Branch Labels} section of
the control panel on the left. Now select \texttt{posterior} from the \texttt{Display} popup menu. The
posterior probabilities won't actually be displayed until you tick the check-box next to the
\texttt{Branch Labels} title. You can then increase the font size here and possibly reduce
the number of displayed significant digits to get the required results.
...
\texttt{Node Bars} section of the control panel and you will notice that it is already
...
Finally, open the \texttt{Appearance} panel and alter the \texttt{Line Weight} to draw the tree with thicker
