...
package dr.app.beauti.components.ebds;
...
/**
 * XML Generator for the EpisodicBirthDeathSampling model and likelihood infrastructure
 *
 * @author Andrew Rambaut
 */

public class EBDSComponentGenerator extends BaseComponentGenerator {

    protected EBDSComponentGenerator(BeautiOptions options) {
        super(options);
    }

    public boolean usesInsertionPoint(InsertionPoint point) {

        EBDSComponentOptions component = (EBDSComponentOptions) options
                .getComponentOptions(EBDSComponentOptions.class);
        ...
        switch (point) {
            case AFTER_TREE_MODEL:          // after the tree model
            case IN_OPERATORS:              // in the operator schedule
            case IN_MCMC_PRIOR:             // in the prior section of the MCMC
            case IN_MCMC_LIKELIHOOD:        // in the likelihood section of the MCMC
            case IN_SCREEN_LOG:             // in the screen log
            case IN_FILE_LOG_PARAMETERS:    // in the file log after the parameters have been logged
                return true;
            default:
                return false;
        }
    ...
    protected void generate(final InsertionPoint point, final Object item, final String prefix, final XMLWriter writer) {
        ...
        switch (point) {
            case AFTER_TREE_MODEL:
                writeEBDSModel(writer);
                break;
            case IN_OPERATORS:
                writeHamiltonianOperatorModel(writer);
                break;
            case IN_MCMC_PRIOR:
                writeEBDSPriors(writer);
                break;
            case IN_MCMC_LIKELIHOOD:
                writeEBDSLikelihood(writer);
                break;
//            case IN_SCREEN_LOG:
...
//                break;
    ...
    private void writeEBDSModel(XMLWriter writer) {
    }

    private void writeScreenLogEntries(XMLWriter writer, EBDSComponentOptions component) {
    }

    private void writeHamiltonianOperatorModel(XMLWriter writer) {

    }

    private void writeEBDSPriors(XMLWriter writer) {

    }

    private void writeEBDSLikelihood(XMLWriter writer) {
...
//    }
    ...
    private void writeLog(XMLWriter writer, EBDSComponentOptions component) {
        for (AbstractPartitionData partition : options.dataPartitions) {
            PartitionSubstitutionModel model = partition.getPartitionSubstitutionModel();
            if (model.isDolloModel()) {
                String prefix = partition.getName() + ".";
                writer.writeIDref(ParameterParser.PARAMETER,
                        component.getOptions().getParameter(prefix + EBDSComponentOptions.DEATH_RATE).getName());
            }
        }
    }
    ...
    protected String getCommentLabel() {
        return "Episodic Birth Death Model";
    }

    private void writeDolloSubstitutionModels(XMLWriter writer, EBDSComponentOptions component) {
    ...
    private void writeDolloSubstitutionModel(AbstractPartitionData partition, XMLWriter writer, EBDSComponentOptions component) {
        String prefix = partition.getName() + ".";
//        String prefix = partition.getPrefix(); // TODO Fix
        writer.writeOpenTag(MutationDeathModelParser.MD_MODEL,
                new Attribute.Default<String>(XMLParser.ID, prefix + EBDSComponentOptions.MODEL_NAME ));
        writeParameter(prefix + EBDSComponentOptions.DEATH_RATE,
                component.getOptions().getParameter(prefix + EBDSComponentOptions.DEATH_RATE), writer);
        writer.writeTag(DataType.DATA_TYPE, new Attribute.Default<String>(XMLParser.IDREF, EBDSComponentOptions.DATA_NAME), true);
        writer.writeCloseTag(MutationDeathModelParser.MD_MODEL);
        writer.write("\n");
    }

    private void writeDolloSiteModel(AbstractPartitionData partition, XMLWriter writer, EBDSComponentOptions components) {
        String prefix = partition.getName() + ".";
        ...
        writer.writeIDref(MutationDeathModelParser.MD_MODEL, prefix + EBDSComponentOptions.MODEL_NAME);
