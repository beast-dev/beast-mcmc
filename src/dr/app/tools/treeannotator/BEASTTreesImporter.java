...
public class BEASTTreesImporter extends Importer implements TreeImporter {
    ...
    private SimpleNode readExternalNode() throws IOException {
        SimpleNode node = new SimpleNode();
        String label = readToken(":(),;");
        node.setNumber(Integer.parseInt(label));
        return node;
    }
     ...
     * return the tree or null if no more trees are available
     */
    public String importNextClades(TreeAnnotator.CladeSystem clades) throws IOException, ImportException {
        if (treeName == null) {
            skipUntilToken("STATE", "_");
            treeName = readToken(",():;");
        }
     ...
     * point at the new node or tip.
     */
    private BitSet readBranch(TreeAnnotator.CladeSystem clades) throws IOException, ImportException {
        BitSet bits = null;
        if (nextCharacter() == '(') {
            // is an internal node
            bits = readInternalNode(clades);

        } else {
            // is an external node
            bits = readExternalNode(clades);
        }
     ...
     * in the node.
     */
    private BitSet readInternalNode(TreeAnnotator.CladeSystem clades) throws IOException, ImportException {
        ...
        // read subsequent children
        while (getLastDelimiter() == ',') {
            BitSet subClade = readBranch(clades);
            bits.or(subClade);
        }
        ...
        try {
            // read a token to get the next delimiter
            readToken(",():;");
            if ((char) getLastDelimiter() == ';') {
                unreadCharacter(';');
            }
     ...
     * Reads an external node in.
     */
    private BitSet readExternalNode(TreeAnnotator.CladeSystem clades) throws IOException {
        String label = readToken(":(),;");
