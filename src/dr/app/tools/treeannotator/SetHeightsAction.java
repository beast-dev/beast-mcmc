...
//        setCladeHeights((BiClade)biclade.getBestRight(), biclade.getRightHeightValues());
    ...
    public void setCladeHeights(BiClade clade, List<Double> heights) {
        ...
        clade.setMeanHeight(getMean(values));
        clade.setMedianHeight(getMedian(values, indices));
        ...
        if (clade.getSize() > minCladeSize && Math.abs(range[0] - range[1]) > HEIGHT_EPSILON && heights.size() >= filterCount) {
            clade.setHeightRange(range);
            clade.setHeightHPD(getHPDs(0.95, values, indices));
        }
    }
    ...
    private static Double[][] getKDE(double interval, double[] values, double lower, double upper) {
        KernelDensityEstimatorDistribution kde = new NormalKDEDistribution(values);
    ...
    public static void main(String[] args) {
        ...
        Double[][] kde = getKDE(0.95, TEST_DATA, range[0], range[1]);
