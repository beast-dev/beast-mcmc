...
public class Parsimony {
	...
	private static void reconstructParsimonyState(Tree tree, NodeRef node, String setName, String attributeName, boolean[] parent) {
		...
		if (!tree.isExternal(node)) {
			...
			if (tree.isRoot(node)) {
				...
				state = (boolean[])((MutableTree)tree).getNodeAttribute(node, setName);

				boolean[] u = union(state, parent);
				boolean[] i = intersection(state, parent);
			...
			((MutableTree)tree).setNodeAttribute(node, attributeName, new Integer(firstIndex(state)));

			for (int i = 0; i < tree.getChildCount(node); i++) {
				reconstructParsimonyState(tree, tree.getChild(node, i), setName, attributeName, state);
			}
		}
	}
	...
	private static boolean[] first(boolean[] s1) {

		boolean[] first = new boolean[s1.length];
