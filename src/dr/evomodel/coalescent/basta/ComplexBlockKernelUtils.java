...
final class ComplexBlockKernelUtils {
    ...
    static void applyPlan(ComplexKernelPlan plan,
                          ...
                          Workspace workspace,
                          int stateCount) {
        applyPlan(plan, transformed, 0, eigenBasisGradient, workspace, stateCount);
    }

    static void applyPlan(ComplexKernelPlan plan,
                          double[] transformed,
                          int transformedOffset,
                          double[] eigenBasisGradient,
                          Workspace workspace,
                          int stateCount) {
        for (ComplexKernelEntry entry : plan.entries) {
            ...
            for (int i = 0; i < entry.leftDim; ++i) {
                int transformedRowOffset = transformedOffset + (entry.leftStart + i) * stateCount;
