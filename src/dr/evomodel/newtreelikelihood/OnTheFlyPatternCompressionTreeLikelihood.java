...
public class OnTheFlyPatternCompressionTreeLikelihood extends AbstractTreeLikelihood {

    public static final String TREE_LIKELIHOOD = "OTFPCTreeLikelihood";
    public static final String USE_AMBIGUITIES = "useAmbiguities";

    /**
     * Constructor.
     */
    public OnTheFlyPatternCompressionTreeLikelihood(PatternList patternList,
                          TreeModel treeModel,
                          SiteModel siteModel,
        ...
        try {
            ...
            logger.info("OTFPC TreeLikelihood");
            ...
            likelihoodCore = new OTFPCGeneralLikelihoodCore(stateCount);
    ...
    protected double calculateLogLikelihood() {
        ...
        if (nodePatterns == null) {
            nodePatterns = new int[nodeCount][patternCount];
            nodePatternCounts = new int[nodeCount];
        }
        calculateSubtreePatterns(treeModel, root);
    ...
    private int[] calculateSubtreePatterns(Tree tree, NodeRef node) {


        int nodeNum = node.getNumber();

        int[] patternIndices = new int[patternCount];
        ...
        if (!tree.isExternal(node)) {
            ...
            // Traverse down the two child nodes
            NodeRef child1 = tree.getChild(node, 0);
            int[] childIndices1 = calculateSubtreePatterns(tree, child1);

            NodeRef child2 = tree.getChild(node, 1);
            int[] childIndices2 = calculateSubtreePatterns(tree, child2);
            ...
            for (int i = 0; i < patternCount; i++) {
                ...
                if (index == null) {
                    ...
                    nodePatterns[child1.getNumber()][j] = childIndices1[i];
                    nodePatterns[child2.getNumber()][j] = childIndices2[i];
                    index = j;
                    map.put(key, index);
                    j++;
                }
                patternIndices[i] = index;
            }
            nodePatternCounts[child1.getNumber()] = map.keySet().size();
            nodePatternCounts[child2.getNumber()] = map.keySet().size();
            likelihoodCore.setNodePatternIndices(child1.getNumber(), nodePatterns[child1.getNumber()], nodePatternCounts[child1.getNumber()]);
            likelihoodCore.setNodePatternIndices(child2.getNumber(), nodePatterns[child2.getNumber()], nodePatternCounts[child2.getNumber()]);

            System.out.println("Node: " + node.getNumber() + " " + nodePatternCounts[child1.getNumber()]);
        } else {
            // we assume the patterns at the tips are just 0, 1, .. stateCount (gap)
            for (int i = 0; i < patternCount; i++) {
                int state  = patternList.getPatternState(nodeNum, i);
                if (state > stateCount) {
                    state = stateCount;
                }
                patternIndices[i] = state;
            }
        }
    ...
    public static XMLObjectParser PARSER = new AbstractXMLObjectParser() {
        ...
        public Object parseXMLObject(XMLObject xo) throws XMLParseException {
            ...
            return new OnTheFlyPatternCompressionTreeLikelihood(
                    patternList,
                    treeModel,
                    siteModel,
                    branchRateModel,
    ...
    int[][] nodePatterns = null;
    int[] nodePatternCounts = null;

    /**
     * the LikelihoodCore
     */
    protected OTFPCLikelihoodCore likelihoodCore;
