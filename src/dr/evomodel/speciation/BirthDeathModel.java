 ...
 * <p/>
 * Comments by Graham Jones in relation to prior at root:
 * <p/>
 * Yang & Rannala (1997) take t_1 to be 1.0 and only make inferences about u
 * and v relative to t_1. The same seems to be true of Nee 2001 which is
 * referenced in the BEAST code.
 * <p/>
 ...
 * different quantity.
 * <p/>
 * For the birth-death model, given s,v,u, then t_1 has density proportional to
 * <p/>
 * (1-E)^(s-2) * ( 1 - (u/v)*E )^(-s-2) * E^2
 * <p/>
 * where E = exp(t*(u-v)), with a more complicated expression if sampling is
 * also modelled. This should be multiplied by Yang and Rannala 97, equation 3
 * to give a value for Pr(t_1,...t_s' | S, s, v, u).
 *
...
public class BirthDeathModel extends SpeciationModel {
    ...
    public void setDeathRate(double deathRate) {
        deathRateParameter.setParameterValue(0, deathRate);
    }

    public double getSamplingProportion() {
        return samplingProportionParameter.getParameterValue(0);
    }

    public void setSamplingProportion(double samplingProportion) {
        samplingProportionParameter.setParameterValue(0, samplingProportion);
    }
    ...
    public double logNodeProbability(Tree tree, NodeRef node) {
        ...
        if (tree.getRoot() == node) {
            ...
            if (rho == 1.0) {
                ...
                double t = tree.getNodeHeight(tree.getRoot());

                int s = tree.getNodeCount();
                double logE = t * (mu - lambda);
                double E = Math.exp(logE);

                // the log of the prior on the root height given by
                // (1-E)^(s-2) * ( 1 - (u/v)*E )^(-s-2) * E^2
                return Math.log(1 - E) * (s - 2) + (Math.log(1.0 - (mu / lambda)) + logE) * (-s - 2) + 2 * logE;
            }
    ...
    private double logPDF(double nodeTime, double rootTime, double lambda, double mu, double rho) {
        ...
        double expmlt = Math.exp((mu - lambda) * nodeTime);
        double term1 = p0tBD(lambda, mu, rho, expmlt);

        double expmlt1 = Math.exp((mu - lambda) * rootTime);
        double term2 = 1.0 - ((p0tBD(lambda, mu, rho, expmlt1) / rho) * expmlt1);

        return Math.log(term1 * term1 * lambda / term2 * rho * expmlt);
