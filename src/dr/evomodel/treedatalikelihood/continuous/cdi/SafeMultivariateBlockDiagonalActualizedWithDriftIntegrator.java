...
import dr.evomodel.treedatalikelihood.continuous.backprop.LyapunovNonStationaryException;
 ...
 *     block-tridiagonal matrix D in basis R.
...
//        transformMatrixBack(Sigma, R);
        ...
        CommonOps.mult(R, SigmaEig, temp);
...
//        }
        ...
        if(DEBUG) {
            System.err.println("\n=== computeOUActualization (BLOCK DIAG) ===");
            System.err.println("scaledOffset = " + scaledOffset);
            System.err.println("Buffer index = " + (scaledOffset / (dimTrait * dimTrait)));
            System.err.println("edgeLength = " + edgeLength);
            System.err.println("blockDParams = " + java.util.Arrays.toString(blockDParams));
            System.err.println("Qeig (in block basis) = " + Qeig);
        }
...
//        transformMatrixBack(Qtrait, R);
        ...
        if(DEBUG)  {
            System.err.println("Qtrait (after R Qeig R^{-1}) = " + workQtrait);
        ...
        if(DEBUG)  {
            System.err.println("Copied to actualizations[" + scaledOffset + "] = " +
                    java.util.Arrays.toString(java.util.Arrays.copyOfRange(actualizations,
                            scaledOffset, scaledOffset + Math.min(9, dimTrait * dimTrait))));
