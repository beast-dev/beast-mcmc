...
//        String la = xo.getAttribute("la", "rowCol");
        Transform.ParsedTransform pt = (Transform.ParsedTransform) xo.getChild(Transform.ParsedTransform.class);
        Transform transform = null;
        if (pt != null) {
            transform = pt.transform;
        }
        Integer subset = null; //TODO implement this
        if (xo.hasAttribute(SUBSET)) {subset = xo.getIntegerAttribute(SUBSET);}
        return new InfinitesimalRatesLogger(substitutionModel, transform, diagonalElements, order, subset);
//        } else {
...
//            AttributeRule.newDoubleRule(SUBSET, true),
