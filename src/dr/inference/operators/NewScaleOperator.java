 ...
 */
public class NewScaleOperator extends AbstractAdaptableOperator {
    private final boolean REJECT_IF_OUT_OF_BOUNDS = true;

    private Parameter indicator;
    private double indicatorOnProb;

    public NewScaleOperator(Parameter parameter, double scale) {

        this(parameter, scale, AdaptationMode.ADAPTATION_ON, 1.0);
    }

    public NewScaleOperator(Parameter parameter, double scale, AdaptationMode mode, double weight) {

        this(parameter, false, 0, scale, mode, weight, null, 1.0, false);
    }

    public NewScaleOperator(Parameter parameter, boolean scaleAll, int degreesOfFreedom, double scale,
