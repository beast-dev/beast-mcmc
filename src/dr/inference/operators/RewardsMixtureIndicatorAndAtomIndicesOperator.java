...
public final class RewardsMixtureIndicatorAndAtomIndicesOperator extends AbstractAdaptableOperator implements Reportable {
    ...
    private final double[] storedIndicatorValues;
    private final double[] storedAtomValues;
    ...
    public RewardsMixtureIndicatorAndAtomIndicesOperator(
        ...
        this.dependentEvidenceProviders =
                new RewardDependentEdgeEvidenceProvider[
                        this.dependentTreeDataLikelihoods.length +
                                this.dependentContinuousTreeDataLikelihoods.length];
        ...
        for (int i = 0; i < this.dependentContinuousTreeDataLikelihoods.length; i++) {
            ...
            if (dependent == null) {
                throw new IllegalArgumentException("dependentContinuousTreeDataLikelihoods contains null at index " + i);
            }
            if (dependent.getTree().getNodeCount() != tree.getNodeCount()) {
                throw new IllegalArgumentException(
                        "Dependent continuous TreeDataLikelihood at index " + i +
                                " must use a tree with the same node count as RewardsAwareBranchModel."
                );
            }
            this.dependentEvidenceProviders[this.dependentTreeDataLikelihoods.length + i] =
        ...
        this.storedIndicatorValues = new double[branchCount];
        this.storedAtomValues = new double[branchCount];
        ...
        this.clusterHelper = useClusterMoves
                ...
                new RewardsMixtureClusterResamplingHelper.ExactLogTargetEvaluator() {
                    ...
                    public double computeCurrentLogTarget(final int[] clusterBranchNodeNumbers, final int clusterCount) {
                        ...
                        for (TreeDataLikelihood dependentTreeDataLikelihood :
                                RewardsMixtureIndicatorAndAtomIndicesOperator.this.dependentContinuousTreeDataLikelihoods) {
                            logTarget += dependentTreeDataLikelihood.getLogLikelihood();
                        }
                        return logTarget;
                    }
    ...
    private double doSingleBranchOperation() {
        ...
        storeCurrentBranchState();
        boolean proposalValid = true;

        if (nToUpdate >= branchCount) {
            for (int parameterIndex = 0; parameterIndex < branchCount; parameterIndex++) {
                proposalValid &= resampleBranchQuietly(parameterIndex, nodeNumberByParameterIndex[parameterIndex]);
            }
            fireParameterChangedEvents();
            return rejectInvalidProposalIfNeeded(proposalValid);
        }
    ...
    private boolean resampleBranchQuietly(final int branchIndex, final int branchNodeNumber) {
        ...
        if (!Double.isFinite(weights.logAtomicTotalWeight) && !Double.isFinite(weights.logCtsWeight)) {
            return false;
        }
    ...
    private RewardsMixtureBranchResamplingHelper.BranchWeights computeBranchWeights(final int branchNodeNumber) {
        ...
        final double dependentContinuousEvidence =
    ...
    private void storeCurrentBranchState() {
        for (int i = 0; i < branchCount; i++) {
            storedIndicatorValues[i] = indicatorZ.getParameterValue(i);
            storedAtomValues[i] = atomIndex.getParameterValue(i);
        }
    }

    private void restoreStoredBranchStateQuietly() {
        for (int i = 0; i < branchCount; i++) {
            indicatorZ.setParameterValueQuietly(i, storedIndicatorValues[i]);
            atomIndex.setParameterValueQuietly(i, storedAtomValues[i]);
        }
        fireParameterChangedEvents();
        makeLikelihoodsDirty();
    }

    private void fireParameterChangedEvents() {
        atomIndex.fireParameterChangedEvent();
        indicatorZ.fireParameterChangedEvent();
    }

    private double rejectInvalidProposalIfNeeded(final boolean proposalValid) {
        if (!proposalValid || !currentLogTargetIsFinite()) {
            restoreStoredBranchStateQuietly();
            return Double.NEGATIVE_INFINITY;
        }
        return 0.0;
    }
    ...
    private boolean currentLogTargetIsFinite() {
        ...
        for (TreeDataLikelihood dependentTreeDataLikelihood : dependentTreeDataLikelihoods) {
            dependentTreeDataLikelihood.makeDirty();
            if (!Double.isFinite(dependentTreeDataLikelihood.getLogLikelihood())) {
                return false;
            }
        }
        for (TreeDataLikelihood dependentTreeDataLikelihood : dependentContinuousTreeDataLikelihoods) {
            dependentTreeDataLikelihood.makeDirty();
            if (!Double.isFinite(dependentTreeDataLikelihood.getLogLikelihood())) {
                return false;
            }
    ...
    private void makeLikelihoodsDirty() {
    ...
    public String getReport() {
        ...
        sb.append("dependentCtmcLikelihoodCount: ").append(dependentTreeDataLikelihoods.length).append("\n");
        sb.append("dependentContinuousLikelihoodCount: ")
                .append(dependentContinuousTreeDataLikelihoods.length).append("\n");
