...
import dr.evomodel.treedatalikelihood.continuous.adapter.CanonicalConjugateRootPriorAdapter;
...
/**
 * Validates the canonical OU tree-passer gradients against numerical finite differences.
 *
 * <p>This mirrors {@link GaussianMessagePasserOUGradientTest}, but targets the new
 * canonical-only OU path built around {@link SequentialCanonicalOUMessagePasser} and
 * {@link HomogeneousCanonicalOUBranchTransitionProvider}. The numerical diffusion check
 * perturbs the variance-form matrix {@code Q} directly, then synchronizes the provider's
 * precision-form diffusion model before evaluating the likelihood.
 */
public class CanonicalGaussianMessagePasserOUGradientTest extends ContinuousTraitTest {

    private static final double FD_DELTA = 1e-5;
    private static final double TOL = 1e-4;
    private static final double BRANCH_TOL = 1e-2;

    private static final double[][] A_VALUES = {
            {2.0, 0.5, 0.1},
            {0.5, 3.0, 0.3},
            {0.1, 0.3, 1.5}
    };

    private static final double[] MU_VALUES = {1.0, -0.5, 2.0};
    ...
    private void checkGradientQ(final OUSetup setup, final String label) {
    ...
    private void checkGradientA(final OUSetup setup, final String label) {
        final int d2 = dimTrait * dimTrait;
    ...
    private void checkGradientMu(final OUSetup setup, final String label) {
        refreshMessages(setup);
    ...
    private void checkGradientBranchLengths(final OUSetup setup, final String label) {
        refreshMessages(setup);
    ...
    private void refreshMessages(final OUSetup setup) {
        syncPrecisionFromVariance(setup.qMatrix, setup.precisionMatrix);
    ...
    private void checkOrthogonalBlockLikelihoodMatchesDense(final OrthogonalBlockOUSetup setup,
                                                            final String label) {
    ...
    private void checkOrthogonalBlockNativeSelectionGradient(final OrthogonalBlockOUSetup setup,
                                                             final String label) {
    ...
    private void checkOrthogonalBlockGradientQMatchesDense(final OrthogonalBlockOUSetup setup,
                                                           final String label) {
        refreshMessages(setup.orthogonal);
    ...
    private OUSetup buildOUSetup(final String tag, final CanonicalTipObservation[] tips) {
    ...
    private OUSetup buildOUSetup(final String tag,
    ...
    private OUSetup buildOUSetupFromElasticModel(final String tag,
                                                 final CanonicalTipObservation[] tips,
    ...
    private OrthogonalBlockOUSetup buildOrthogonalBlockOUSetup(final String tag,
                                                               final CanonicalTipObservation[] tips) {
    ...
    private CanonicalTipObservation[] buildFullyObservedTips() {
    ...
    private CanonicalTipObservation[] buildPartiallyObservedTips() {
    ...
    private Map<String, double[]> buildStandardTraitValues() {
    ...
    private static void fillMatrixParameter(final MatrixParameter matrix, final double[][] values) {
    ...
    private static void syncPrecisionFromVariance(final MatrixParameter qMatrix,
    ...
    private static final class OUSetup {
        final MatrixParameter aMatrix;
    ...
    private static final class OrthogonalBlockOUSetup {
        final OrthogonalBlockDiagonalPolarStableMatrixParameter blockSelection;
    ...
    private static final class ScaledBranchRateModel extends AbstractBranchRateModel {
