...
branches with fast rates are followed by branches with fast rates. This statistic measure the covariance between parent
and child branch rates in you tree in a relaxed molecular clock analysis. If this value spans zero, then branches with
...
data set attached to this manual.
...
\xml{idref} attribute to refer to the parameter in subsequent elements. To see how multi-locus data sets are defined in
...
This program is for summarizing the information in a sample of trees produced by BEAST, e.g., annotating the probability
of nodes having a rate change.  Once again, when you open this program the TreeAnnotator user interface window and the
JAVA TreeAnnotator window will appear.
\subsection{BurnIn}
This option allows you to select the amount of BurnIn, i.e., the number of samples that will be discarded at the start
of the run, so that you are only analysing data that has reached equilibrium.
\subsection{Posterior probability limit}
This is the same as specifying a a limit for bootstrapping in PAUP*.  It will only show branches within the tree that
have a posterior probability of greater than 0.5, and collaspe any branches into polytomies that have a posterior
probability limit of less than 0.5.
\subsection{Target tree type}
Maximum clade credibility and User target tree.  Explain?
\subsection{Node heights}
This option allows you select how the node heights are summarised on the the tree, whether you keep the target heights,
or display mean or median heights.
