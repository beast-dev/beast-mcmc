...
\begin{itemize}
\item \textbf{\textsc{\texttt{BEAUti}}} is a software package written in Java and
...
\item \textbf{\textsc{\texttt{Tracer}}} is a software package written in Java and
...
as \texttt{MrBayes} \cite{HR2001} and \textbf{\textsc{\texttt{Bali-Phy}}}
\cite{SR2006}.
\end{itemize}
...
the Bayes Factor (BF), which is the ratio of their marginal likelihoods. Generally speaking calculating the BF involves 
a Bayesian MCMC that averages over both models (using a technique called `reversible jump' MCMC), and this is not something 
...
\section{The \texttt{BEAST} software package}
...
\cite{?????}. The aim is to obtain an estimate of the rate of molecular evolution, an estimate of the date of 
...
the current version is v1.4.4. It is available for download from http://\texttt{BEAST}.bio.ed.ac.uk/.
...
information. At the time of writing, the current version is v1.4. It is available for download from http://\texttt{BEAST}.bio.ed.ac.uk/.
...
\section{Analysing the \texttt{BEAST} output}
...
go for a chain length of 5,000,000. Go back to Section~\ref{section:Setting the MCMC options}
...
input format, common questions and error messages.
 ...
 http://\texttt{BEAST}.bio.ed.ac.uk/
 \item Tutorials:
 http://\texttt{BEAST}.bio.ed.ac.uk/Tutorials/
 \item Frequently asked questions:
 http://\texttt{BEAST}.bio.ed.ac.uk/FAQ/
 \end{itemize}
