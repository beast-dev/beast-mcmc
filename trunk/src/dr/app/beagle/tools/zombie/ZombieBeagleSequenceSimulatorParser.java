 ...
 * @version $Id$
 */
public class ZombieBeagleSequenceSimulatorParser extends AbstractXMLObjectParser {

	public static final String BEAGLE_SEQUENCE_SIMULATOR = "oldBeagleSequenceSimulator";
	...
	public XMLSyntaxRule[] getSyntaxRules() {
		...
		return new XMLSyntaxRule[] {
				...
				new ElementRule(ZombiePartition.class, 1, Integer.MAX_VALUE)
				};
	...
	public Object parseXMLObject(XMLObject xo) throws XMLParseException {
		...
		for (int i = 0; i < xo.getChildCount(); i++) {
			...
			ZombiePartition partition = (ZombiePartition) xo.getChild(i);
			
			if (partition.from > replications) {
				throw new XMLParseException(
						"illegal 'from' attribute in " + ZombiePartitionParser.PARTITION + " element");
			}

			if (partition.to > replications) {
				throw new XMLParseException(
						"illegal 'to' attribute in " + ZombiePartitionParser.PARTITION + " element");
			}
...
//			msg += "\n\t" + "partition" + (i + 1) + " from " + partition.from + " to " + partition.to + " every " + partition.every;
		...
		ZombieBeagleSequenceSimulator s = new ZombieBeagleSequenceSimulator(partitionsList, //
				replications //
		);
