 ...
 * @version $Id: DemographicPlotPanel.java,v 1.9 2005/07/11 14:07:26 rambaut Exp $
 */
public class DemographicPlotPanel extends JPanel {
	...
	private JCheckBox logAxisCheckBox = new JCheckBox("Log axis");
	...
	public DemographicPlotPanel() {		
		setMinimumSize(new Dimension(300,150));
		setLayout(new BorderLayout());

		logAxisCheckBox.setSelected(true);
		logAxisCheckBox.setEnabled(false);
		...
		toolBar.add(logAxisCheckBox);

        toolBar.add(new JToolBar.Separator(new Dimension(8,8)));
        toolBar.add(solidIntervalCheckBox);

        label.setFont(logAxisCheckBox.getFont());
        meanMedianComboBox.setFont(logAxisCheckBox.getFont());
		...
		logAxisCheckBox.addItemListener(
			new java.awt.event.ItemListener() {
				public void itemStateChanged(java.awt.event.ItemEvent ev) {
					updatePlots();
				}
	...
	public void setupPlot(String title, Variate xData, 
		...
		logAxisCheckBox.setEnabled(true);
	...
	public void updatePlots() {
		...
		if (logAxisCheckBox.isSelected()) {
			demoChart.setYAxis(new LogAxis());
		} else {
			demoChart.setYAxis(new LinearAxis());
		}
