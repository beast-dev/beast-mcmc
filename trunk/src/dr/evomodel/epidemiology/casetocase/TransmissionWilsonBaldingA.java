...
public class TransmissionWilsonBaldingA extends AbstractTreeOperator {
    ...
    public void proposeTree() throws OperatorFailedException {
        ...
        Integer[] possibleDestinations = c2cLikelihood.samePainting(iP,false);

        NodeRef k = tree.getNode(possibleDestinations[MathUtils.nextInt(possibleDestinations.length)]);
        NodeRef j = tree.getChild(k, MathUtils.nextInt(1));

        while ((tree.getNodeHeight(k) <= tree.getNodeHeight(i)) || (i == j)) {
            k = tree.getNode(possibleDestinations[MathUtils.nextInt(possibleDestinations.length)]);
            j = tree.getChild(k, MathUtils.nextInt(1));
        }
    ...
    private boolean eligibleForMove(NodeRef node, TreeModel tree, AbstractCase[] branchMap){
        ...
        // the parent has no effect on the transmission tree) and there must be at least three nodes in that partition
        // (because the parent needs a new node to become the child of)

        return branchMap[tree.getParent(node).getNumber()]==branchMap[tree.getParent(tree.getParent(node)).getNumber()]
                && c2cLikelihood.samePainting(tree.getParent(node),false).length>2;
    }
