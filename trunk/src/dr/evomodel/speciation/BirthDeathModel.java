...
public class BirthDeathModel extends SpeciationModel {
	...
	public double logNodeHeightProbability(double nodeHeight, double rootHeight) {
		...
		// @todo do Yang and Rannala (1997) have the root time constrained to be 1.0?
//		double t1 = nodeHeight / rootHeight;
//		double t0 = 1.0;
        double t1 = nodeHeight;
        double t0 = rootHeight;

		return logPDF(t1, t0, lambda, mu, rho);
	}
...
//   species sampling.
		...
		if (Math.abs(lambda - mu) < small) {
			...
			return (1+rho*lambda * rootTime)/(rootTime * term * term);
		}
