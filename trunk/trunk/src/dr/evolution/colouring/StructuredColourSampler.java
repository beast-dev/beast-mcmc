...
public class StructuredColourSampler implements ColourSampler {
	...
	private void computeIntervals( Tree tree, MetaPopulationModel mp ) {
...
//    public TreeColouring sampleTreeColouring(Tree tree, ColourChangeMatrix colourChangeMatrix, double[] N) {
    ...
    double[] pruneBranchEM(ColourChangeMatrix mm, double[] inState, NodeRef parent, NodeRef child, Tree tree, MetaPopulationModel mp) {
     ...
     */
    private double[] pruneEM(Tree tree, NodeRef node, ColourChangeMatrix mm, MetaPopulationModel mp) {
 ...
 */
     ...
     * Requires the results from Felsenstein Backwards pruning, in nodePartials(EM)[] (see pruneEM())
     */
    private double calculateEMProposal(Tree tree, NodeRef node, ColourChangeMatrix mm, MetaPopulationModel mp, TreeColouring colouring) {
